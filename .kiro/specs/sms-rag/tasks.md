...
- [ ] 1. Set up project structure and shared infrastructure
  ...
  - [~] 1.2 Implement configuration system (`sms_rag/shared/config.py`)
  ...
  - [~] 1.3 Write property test for configuration validation (Property 17)
    - **Property 17: Configuration Validation Errors**
    - **Validates: Requirements 9.6**

  - [~] 1.4 Implement data models and interfaces (`sms_rag/shared/`)
...
- [ ] 2. Implement embedding model and vector store
  - [~] 2.1 Implement embedding model wrapper (`sms_rag/shared/embedding.py`)
  ...
  - [~] 2.2 Implement ChromaDB adapter (`sms_rag/shared/chroma_store.py`)
  ...
  - [~] 2.3 Write property test for vector store metadata round-trip (Property 7)
    - **Property 7: Vector Store Metadata Round-Trip**
    - **Validates: Requirements 4.3**

- [ ] 3. Implement PDF parser and conversation chunker
  - [~] 3.1 Implement PDF parser (`sms_rag/preprocessing/pdf_parser.py`)
  ...
  - [~] 3.2 Write property tests for PDF parsing (Properties 1, 2, 3)
  ...
  - [~] 3.3 Implement conversation chunker (`sms_rag/preprocessing/chunker.py`)
  ...
  - [~] 3.4 Write property tests for chunking (Properties 5, 6)
...
- [ ] 5. Implement BM25 index and hybrid retriever
  - [~] 5.1 Implement BM25 keyword search index (`sms_rag/preprocessing/bm25_builder.py` and `sms_rag/query/bm25_search.py`)
  ...
  - [~] 5.2 Implement hybrid retriever (`sms_rag/query/retriever.py`)
  ...
  - [~] 5.3 Write property tests for hybrid retrieval (Properties 8, 9, 10)
...
- [ ] 6. Implement LLM providers and orchestrator
  - [~] 6.1 Implement Ollama LLM provider (`sms_rag/query/ollama_provider.py`)
  ...
  - [~] 6.2 Implement Bedrock LLM provider (`sms_rag/query/bedrock_provider.py`)
  ...
  - [~] 6.3 Implement RAG orchestrator (`sms_rag/query/orchestrator.py`)
  ...
  - [~] 6.4 Write property tests for orchestrator (Properties 11, 12)
    - **Property 11: Orchestrator Context Chunk Cap**
    - **Property 12: Orchestrator Output Completeness**
    - **Validates: Requirements 6.5, 6.7**

- [ ] 7. Implement session store and chat interface
  - [~] 7.1 Implement session store (`sms_rag/query/session_store.py`)
  ...
  - [~] 7.2 Write property tests for session store (Properties 18, 19, 20)
  ...
  - [~] 7.3 Implement chat interface (`sms_rag/query/chat_ui.py`)
  ...
  - [~] 7.4 Write property tests for chat interface (Properties 13, 14)
...
- [ ] 9. Implement preprocessing pipeline entry point
  - [~] 9.1 Implement preprocessing pipeline (`sms_rag/preprocessing/pipeline.py`)
  ...
  - [~] 9.2 Create preprocessing entry point (`sms_rag/preprocessing/__main__.py`)
  ...
  - [~] 9.3 Write property tests for preprocessing pipeline (Properties 4, 15, 16)
...
- [ ] 10. Implement query pipeline entry point and wire everything together
  - [~] 10.1 Create query pipeline entry point (`sms_rag/query/__main__.py`)
  ...
  - [~] 10.2 Wire preprocessing pipeline components
  ...
  - [~] 10.3 Wire query pipeline components
